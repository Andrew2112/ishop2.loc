...
class Pagination
{
    ...
    public function getHtml(){
        ...
        $startpage=null;
        $endpage=null;
        $page2left=null;//вторая стр слева
        $page1left=null;//первая стр слева
        $page2right=null;
        $page1right=null;
        ...
        if ($this->currentPage + 2 <= $this->countPages){
            $page1left="<li><a class='nav-link' href='{$this->uri}page=" .($this->currentPage + 2). "'>" . ($this->currentPage + 2). "</a></li>";

        }
        return '<ul class="pagination">' . $startpage.$back.$page2left. '<li class="active"><a>'.$this->currentPage.'</a></li>'.$page1right.$page2right.$forward.$endpage.'</ul>';
